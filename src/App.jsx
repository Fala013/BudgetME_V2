...
import { collection, addDoc, getDocs, query, where, onSnapshot, doc, setDoc, deleteDoc } from 'firebase/firestore';
...
const ICONS = {
    ...
    settings: (
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M10 6V10L13 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
    ),
    eye: (
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M1 10C2.73 5.61 6.36 3 10 3C13.64 3 17.27 5.61 19 10C17.27 14.39 13.64 17 10 17C6.36 17 2.73 14.39 1 10Z" stroke="currentColor" strokeWidth="2"/>
            <circle cx="10" cy="10" r="3" stroke="currentColor" strokeWidth="2"/>
        </svg>
    ),
    eyeOff: (
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M1 10C2.73 5.61 6.36 3 10 3C11.13 3 12.23 3.19 13.26 3.54M19 10C17.27 14.39 13.64 17 10 17C8.87 17 7.77 16.81 6.74 16.46M4.22 4.22L15.78 15.78" stroke="currentColor" strokeWidth="2"/>
            <circle cx="10" cy="10" r="3" stroke="currentColor" strokeWidth="2"/>
        </svg>
    ),
    user: (
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <circle cx="10" cy="8.5" r="3" stroke="#444" strokeWidth="2" fill="none"/>
            <path d="M5.5 15.5c1.5-2 7.5-2 9 0" stroke="#444" strokeWidth="2" fill="none"/>
        </svg>
    ),
};
...
function App() {
    ...
    useEffect(() => {
      ...
      const unsubscribeTx = onSnapshot(qTx, (querySnapshot) => {
        const txs = [];
    ...
    const handleAddAccount = async () => {
        try {
            ...
            if (editingAccountId) {
                await setDoc(doc(db, 'accounts', String(editingAccountId)), {
                ...
                await setDoc(doc(db, 'accounts', String(newId)), {
                    name: accountForm.name.trim(),
                    balance: parseFloat(accountForm.balance),
                    userId: user.uid,
                    id: newId
    ...
    const handleAddOrEdit = async () => {
        ...
        try {
            ...
            if (editingId) {
                await setDoc(doc(db, 'transactions', String(editingId)), updated);
                setEditingId(null);
            } else {
    ...
    const handleDeleteConfirm = async () => {
        try {
            await deleteDoc(doc(db, 'transactions', String(deleteId)));
            setDeleteId(null);
    ...
    const handleDeleteAccountConfirm = async () => {
        try {
            await deleteDoc(doc(db, 'accounts', String(deleteAccountId)));
            setShowDeleteAccountModal(false);
            setDeleteAccountId(null);
        } catch (err) {
            setError('Errore nell\'eliminazione conto: ' + err.message);
        }
    };
    ...
    const getAccountBalance = (account) => {
        // Data di oggi in formato YYYY-MM-DD (UTC)
        const todayISO = new Date().toISOString().split('T')[0];
        const txSum = transactions
            .filter(t => t.account === account.name)
            .filter(t => {
                // Prendi la data della transazione in formato YYYY-MM-DD
                let txDateISO = '';
                if (t.displayDate) {
                    txDateISO = new Date(t.displayDate).toISOString().split('T')[0];
                } else if (t.date) {
                    txDateISO = new Date(t.date).toISOString().split('T')[0];
                } else {
                    return false;
                }
                // Includi solo le transazioni fino a oggi (in UTC)
                return txDateISO <= todayISO;
            })
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            setError('Errore di login: ' + err.message);
        }
    };
        ...
        return (
            <div className="container" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                <div className="loginCard">
                    <h1 style={{ textAlign: 'center', marginBottom: 32 }}>BudgetME</h1>
                    <form onSubmit={handleLogin} style={{ width: '100%', maxWidth: 340, display: 'flex', flexDirection: 'column', gap: 18 }}>
                        <input
                        ...
                        <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                            <button className="button saveButton loginBtn" type="submit">Login</button>
                            <button className="button cancelButton loginBtn" type="button" onClick={handleSignup}>Registrati</button>
                        </div>
                    </form>
                    {error && <p className="error" style={{ marginTop: 18, textAlign: 'center' }}>{error}</p>}
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="container">
            ...
            {showAccountMenu && (
                <div className="modalOverlay" onClick={() => setShowAccountMenu(false)}>
                    <div className="modal" onClick={e => e.stopPropagation()}>
                        <div className="modalHeader">
                            <h2 className="modalTitle">Gestione Account</h2>
                            <button className="closeButton" onClick={() => setShowAccountMenu(false)}>{ICONS.close}</button>
                        </div>
                        <div style={{ marginBottom: 20, fontSize: 16 }}>
                            <div><b>Email:</b> {user?.email}</div>
                        </div>
                        <div className="buttonContainer">
                            <button className="button deleteButton" onClick={handleLogout}>Logout</button>
                        </div>
                    </div>
                </div>
            )}
